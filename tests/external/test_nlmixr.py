...
def test_pheno_error_model(testdata, load_model_for_test):
    ...
    assert "add_error <- 0" in model.code
    assert "prop_error <- SIGMA_1_1" in model.code
    assert "Y ~ add(add_error) + prop(prop_error)" in model.code


def test_prop_alias(testdata, load_model_for_test):
    nmmodel = load_model_for_test(testdata / 'nonmem' / 'models' / 'mox2.mod')
    model = convert_model(nmmodel)
    assert "prop_error <- RUV_PROP" in model.code
...
def test_remove_sigma(testdata, load_model_for_test):
    ...
    model = convert_model(nmmodel)
    assert "SIGMA_1_1" not in model.code


def test_sigma(testdata, load_model_for_test):
    nmmodel = load_model_for_test(testdata / 'nonmem' / 'models' / 'mox2.mod')
    model = convert_model(nmmodel)
    assert "RUV_PROP <- c(0.0, 0.1, Inf)" in model.code
...
def test_w_error_model(testdata, load_model_for_test):
    ...
    model = convert_model(nmmodel)
    model
    assert 'add_error <- sqrt(THETA_3**2)' in model.code
    assert 'prop_error <- sqrt(THETA_4**2)' in model.code
    assert "Y ~ add(add_error) + prop(prop_error)" in model.code

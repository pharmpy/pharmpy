...
def test_derivative_model(load_model_for_test, testdata):
    ...
    assert len(modelentry.model.estimation_steps[0].derivatives) == 5
    assert (
        modelentry.model.model_code
...
$TABLE ID TIME DV G011 G021 H011 FILE=mytab NOAPPEND NOPRINT
...
def test_linearized_model(load_model_for_test, testdata):
    ...
    datainfo = DataInfo.create(
        ['D_ETA1', 'D_ETA2', 'D_EPS1', 'D_EPSETA1_1', 'D_EPSETA1_2', 'OETA1', 'OETA2', 'OPRED']
    )

...
def test_retries(tmp_path, model_count, scale, use_initial, start_modelres):
    with chdir(tmp_path):
        fraction = 0.1
        ...
        for model in res.models:
            if model != start_modelres[0]:
                is_within_fraction(
                    start_modelres[0], start_modelres[1], model, scale, fraction, use_initial
                )
...
def is_within_fraction(start_model, start_res, candidate_model, scale, fraction, use_initial):
    if use_initial:
        parameter_value = [(p.name, p.init) for p in start_model.parameters]
    else:
        parameter_value = list(start_res.parameter_estimates.items())
    ...
    if scale == "normal":
        ...
        if not use_initial:
            start_model = update_inits(start_model, start_res.parameter_estimates)

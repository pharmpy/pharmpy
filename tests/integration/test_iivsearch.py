...
from pharmpy.tools import fit, read_modelfit_results, retrieve_models, run_iivsearch
...
def test_no_of_etas_keep_dummy(tmp_path, load_model_for_test, testdata):
...
@pytest.mark.skipif(tflite_condition, reason="Skipping tests requiring tflite for Python 3.12")
def test_block_structure_dummy(tmp_path, load_model_for_test, testdata):
    model = load_model_for_test(testdata / 'nonmem' / 'models' / 'mox2.mod')
    results = read_modelfit_results(testdata / 'nonmem' / 'models' / 'mox2.mod')
    with chdir(tmp_path):
        res = run_iivsearch(

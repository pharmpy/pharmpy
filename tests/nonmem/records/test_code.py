...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('$PK CL = PLOG(THETA(1))', S('CL'), sympy.log(S('THETA(1)'))),
        ('$PK CL = PLOG10(THETA(1))', S('CL'), sympy.log(S('THETA(1)'), 10)),
        ('$PRED CL = EXP(2)', S('CL'), sympy.exp(2)),
        ('$PRED CL = PEXP(2)', S('CL'), sympy.exp(2)),
        ('$PRED CL = exp(2)', S('CL'), sympy.exp(2)),
        ('$PRED cL = eXp(2)', S('CL'), sympy.exp(2)),
        ('$PRED CL = DEXP(ETA(1))', S('CL'), sympy.exp(S('ETA(1)'))),
        ('$PRED CL = PDZ(ETA(1))', S('CL'), 1 / S('ETA(1)')),
        ('$PRED CL = PZR(ETA(1))', S('CL'), S('ETA(1)')),
        ('$PRED CL = PNP(ETA(1))', S('CL'), S('ETA(1)')),
        ('$PRED CL = PHE(ETA(1))', S('CL'), S('ETA(1)')),
        ('$PRED CL = PNG(ETA(1))', S('CL'), S('ETA(1)')),
        ...
        ('$PRED CL = PSQRT(X)', S('CL'), sympy.sqrt(S('X'))),

...
def test_resampler():
    ...
    with pytest.raises(ValueError) as e:
        resampler = data.Resample(df, 'ID', replace=False, sample_size=4)
        next(resample)

    resampler = data.Resample(df, 'ID', replace=False, sample_size=3)
    next(resampler)

    resampler = data.Resample(df, 'ID', stratify='STRAT')
    (new_df, ids) = next(resampler)
    assert list(ids) == [1, 4, 2]
    assert list(new_df['ID']) == [1, 1, 2, 2, 3, 3]
    assert list(new_df['DV']) == [5, 6, 0, 9, 3, 4]

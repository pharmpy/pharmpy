...
def get_covariate_effects(model: Model) -> dict[list]:
    ...
    covariates = [c for c in covariates if model.datainfo[str(c)].type in ["covariate", "unknown"]]
    ...
    for param, covariates in param_w_cov.items():
        for cov in covariates:
            effop = _get_covariate_effect(model, param, cov)
            if effop:
                res[(param, cov)].append((effop[0], effop[1]))
    return res

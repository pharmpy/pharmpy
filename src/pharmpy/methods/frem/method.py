...
def check_covariates(input_model, covariates):
    ...
    if unique_covariates:
        warnings.warn(f'The covariates {list(unique_covariates)} have the same baseline value for '
    ...
    for col1, col2 in itertools.combinations(covset, 2):
        if cov_bls[col1].equals(cov_bls[col2]):
            ...
            warnings.warn(f'The baselines for covariates {col1} and {col2} are equal for all '

...
def samba_workflow(
    ...
    #                          p_forward, max_steps,
    #                          # strictness, adaptive_scope_reduction,
    #                          )
...
def _init_search_state(
    ...
    exploratory_cov_funcs, linear_cov_funcs, filtered_model = filter_search_space_and_model(search_space, model)  # exploratory effect functions, filtered model (cleaned cov effects)
    ...
    nonlinear_search_state = SearchState(modelentry, filtered_modelentry, nonlinear_model_candidate, [nonlinear_model_candidate])
    
    # linear search states {param: SearchState}
    # linear_search_states = _init_linear_search_state(linear_cov_funcs, filtered_modelentry)
    param_indexed_func_list = {}
    param_cov_list = {}
    for cov_effect, cov_func in linear_cov_funcs.items():
        param = cov_effect[0]
        if param not in param_indexed_func_list.keys():
            param_indexed_func_list[param] = {cov_effect: cov_func}
            param_cov_list[param] = [cov_effect[1]]
        else:
            param_indexed_func_list[param].update({cov_effect: cov_func})
            param_cov_list[param].append(cov_effect[1])
    linear_search_states = dict.fromkeys(param_cov_list.keys(), None)
    ...
    for param, covariates in param_cov_list.items():
        proxy_model_dict[param] = []
        param_base_model = _create_samba_base_model(filtered_modelentry, param, covariates)
        param_base_modelentry = ModelEntry.create(model=param_base_model)  # update dataset and fit all linear models in samba step
        proxy_model_dict[param].append(param_base_modelentry)
        
        # create linear covariate models for each parameter ("lin", "+")
        for cov_effect, linear_func in param_indexed_func_list[param].items():
        ...
        # create search state for each parameter
        candidates = [Candidate(me, steps=(SambaStep(alpha, DummyEffect("", "", "", "")),))
                      for me in proxy_model_dict[param]]
        linear_search_states[param] = SearchState(filtered_modelentry, param_base_modelentry, candidates[0], candidates)
    
    
    return (nonlinear_search_state, linear_search_states, exploratory_cov_funcs, param_cov_list)

    
# init linear search state first, than use model = model.replace(dataset) to update the linear search state

def samba_step(state_and_funcs):
    nonlinear_search_state, linear_search_states, exploratory_cov_funcs, param_cov_list = state_and_effect
    ...
    for param, covaraites in param_cov_list.items():
        # update dataset
        updated_dataset = _create_samba_dataset(best_nlme_modelentry, param, covaraites)
        # update candidates
        all_candidates = linear_search_states[param].all_candidates_so_far
        new_candidates = [replace(candidate, 
                                  modelentry=ModelEntry.create(
                                      model=candidate.modelentry.model.replace(dataset=updated_dataset))
                                  )
                          for candidate in all_candidates]
        # update linear search state
        linear_search_states[param] = replace(linear_search_states[param],
                                              best_candidate_so_far=new_candidates[0],
                                              all_candidates_so_far=new_candidates)
    
    # fit linear covariate models
    # FIXME: MINIMIZATION TERMINATED with potentially wrong OVF values
    for param, search_state in linear_search_states.items():
        candidates = search_state.all_candidates_so_far
        candidates = [replace(candidate, 
                              modelentry=candidate.modelentry.attach_results(fit(candidate.modelentry.model)))
                      for candidate in candidates]
        search_state = replace(search_state, all_candidates_so_far=candidates)
        linear_search_states[param] = search_state
    
    # covariate model selection: best of many
    selected_explor_cov_funcs = []
    for param, search_state in linear_search_states.items():
        param_base_model = search_state.best_candidate_so_far
        candidates = search_state.all_candidates_so_far
        candidate_modelentries = list(map(lambda candidate: candidate.modelentry, candidates))
        ofvs = [candidate.modelentry.modelfit_results.ofv 
                if candidate.modelentry.modelfit_results is not None 
                else np.nan
                for candidate in candidates]
        selected_cov = lrt_best_of_many(candidate_modelentries[0], candidate_modelentries[1:], ofvs[0], ofvs[1:], alpha=0.05)
        # FIXME: if param_base_model is returned, stop searching and return the final model; 
        #  else continue the rest nonlinear model selection steps
    ...
    if any(selected_explor_cov_funcs):
        ...
        new_nonlin_modelentries = [ModelEntry.create(model=model, modelfit_results=result) for model, result in zip(new_nonlin_models, new_model_results)]
...
def samba_search(max_steps, state_and_funcs):
    steps = range(1, max_steps+1)
    for step in steps:
        state_and_effect = samba_step(state_and_funcs)
    pass 
...
def _create_samba_dataset(model_entry, param, covariates):
    ...
    covariate_columns.loc[:,covariates] = covariate_columns.loc[:,covariates].apply(np.log)  # for linear proxy model, covariates need to be log transformed
